...
function productData(b:any){const category=text(b.category,'Category');if(!CATEGORIES.includes(category))fail(400,'Choose one of the shop categories.');return [text(b.name,'Product name',160),text(b.description||'','Description',10000,false),text(b.brand||'','Brand',100,false),category,price(b.price),text(b.unit,'Selling unit',40),integer(b.quantity,'Stock quantity'),b.active?1:0];}
...
async function handler(req:Request){try{
 ...
 if(path[0]==='products'&&method==='GET'){
  ...
  const q=(url.searchParams.get('q')||'').slice(0,150),cat=url.searchParams.get('category')||'',offset=Math.max(0,Math.min(Number(url.searchParams.get('offset'))||0,1e7));const seed=Math.abs(Number(url.searchParams.get('seed'))||731)%1000000;const brand=url.searchParams.get('brand')||'';const where='p.active=1 AND (?=\'\' OR p.category=?) AND (?=\'\' OR p.brand=?) AND (?=\'\' OR p.name LIKE ? ESCAPE \'\\\' OR p.description LIKE ? ESCAPE \'\\\' OR p.brand LIKE ? ESCAPE \'\\\')';const pattern='%'+q.replace(/[\\%_]/g,'\\$&')+'%';const args=[cat,cat,brand,brand,q,pattern,pattern,pattern];const products=await all(`SELECT ${productColumns} FROM products p WHERE ${where} ORDER BY ((p.sort_key*48271+?) % 2147483647),p.id LIMIT 25 OFFSET ?`,...args,seed,offset);return json({products:products.slice(0,24),more:products.length>24});
 }
 ...
 if(path[0]==='admin'){
  ...
  if(path[1]==='products'){
   ...
   if(method==='POST'&&!path[2]){const data=productData(b),id=crypto.randomUUID();await run('INSERT INTO products(name,description,brand,category,price,unit,quantity,active,id,sort_key,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)',...data,id,crypto.getRandomValues(new Uint32Array(1))[0]%2147483647,Date.now());return json(await product(id,true),201);}
   if(method==='PATCH'&&path[2]){const data=productData(b),version=integer(b.version,'Product version',1);const res=await run('UPDATE products SET name=?,description=?,brand=?,category=?,price=?,unit=?,quantity=?,active=?,version=version+1 WHERE id=? AND version=?',...data,path[2],version);if(!res.meta.changes)fail(409,'This product or its stock changed. Reload the product before saving.');return json(await product(path[2],true));}
